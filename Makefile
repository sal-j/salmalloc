...
$(TEST_BUILD)/%: $(TEST_DIR)/%.c
	$(CC) $(CFLAGS_GDB) $(CFLAGS_SHARED_LIB)$(SHARED_LIB_DIR) $(CFLAGS_INCLUDE)$(INCLUDE_DIR) $< $(CFLAGS_OUT) $@ $(CFLAGS_LIBS_FLAG)$(LIB_NAME)

$(TSTS): $(SHARED_LIB_0)
	$(CC) $(CFLAGS_SHARED) $(BUILDDIR)/$(SHARED_LIB_0) $(CFLAGS_OUT) $(BUILDDIR)/$(LIB_0)

$(SHARED_LIB_0) : $(SRCDIR)/$(SRC)
	$(CC) $(CFLAGS_WARN_ALL) $(CFLAGS_INCLUDE)$(INCLUDE_DIR) $(CFLAGS_C) $(SRCDIR)/$(SRC1) $(CFLAGS_OUT) $(BUILDDIR)/$(SHARED_LIB_0)
